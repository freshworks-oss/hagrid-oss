...
In this section, we are going to do playground setup. Basically a setup which allow developers to understand and play around with various features of Hagrid without much infra level setup. 
...
So now, go to Hagrid repository at https://github.com/freshworks-oss/hagrid-oss, switch to branch `hagrid/initializer` 
...
2. `Grafana` AND `Prometheus` - It is used to view metrics that Hagrid fetches along with it. 
3. `Hagrid` - This container checkout branches that are passed via `docker-compose.yml` and run test cases on that branch. 
   ...
   2. We need this container only during development of `Hagrid` framework itself. 
4. `mongodb` - Hagrid internally uses two kind of storage to store beans and assets 
   1. `persistent` - Which means Hagrid should use `mongodb` as its infra layer
   2. `inmemory` -  Which means Hagrid should use `RAM` as its infra layer. 
...
If so then please import the dashboard from this json [https://github.com/freshworks-oss/hagrid-oss/blob/main/src/main/resources/grafana_dashboard.json](grafana Dashboard json)
