...
## Added
### Support for Sync Termination from Steps & Main App Code  

We have added the support to terminate Hagrid sync from anywhere where developer has access to `SyncService`. 
For this, we have added a method `shutdown` in `SyncService`. This method takes the argument in seconds, how long should it wait
for graceful termination. 

```java

SyncServiceContainer syncServiceContainer = syncService.startSync(ParentStep.class, "sync_test_namespace", x);
SyncStatusService syncStatusService = syncServiceContainer.getBean(SyncStatusService.class);

// Once syncService instance is available, then shutdown after waiting for 10 seconds. 
syncService.shutdown(10);

        
```

### Added Prometheus Metrics for HttpClient and Mongo Client 
